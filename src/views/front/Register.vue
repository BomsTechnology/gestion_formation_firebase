...
const { loading, createUser, errors, cleanErrors, loginWithGoogle } =
  useUsers();
...
const storeUser = async () => {
  if (user.password === user.confirmation_password) {
    ...
    errors.value = "Passwords must be identical";
  }
};
</script>
<template>
  <GoBack />
  <div class="w-full max-w-lg p-6 m-auto bg-white rounded-md py-4">
    <h1
    ...
    <div class="flex items-center mt-8 -mx-2">
      ...
      >
        ...
        <span class="hidden mx-2 sm:inline">Register with Google</span>
      </button>
    </div>
    ...
    <form class="mt-6" @submit.prevent="storeUser()">
      <div class="grid grid-cols-2 gap-4">
        <div class="">
          <label for="username" class="block text-sm text-gray-800">Name</label>
          <input
            type="text"
            v-model="user.name"
            required
        ...
        <div class="">
          <label for="" class="block text-sm text-gray-800">Phone</label>
          <input
            type="text"
            v-model="user.phone"
            required
        ...
        <div class="col-span-2">
          <label for="" class="block text-sm text-gray-800">Email</label>
          <input
            type="email"
            v-model="user.email"
            required
        ...
        <div class="">
          <label for="" class="block text-sm text-gray-800"
      ...
      <div class="mt-6">
        <button
          class="w-full px-4 py-2 flex justify-center tracking-wide text-white transition-colors duration-200 transform bg-blue-500 rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600"
        >
          <span v-if="loading == 1">
            <Spin />
          </span>
